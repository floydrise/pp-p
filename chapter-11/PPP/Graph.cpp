...
namespace Graph_lib {
    ...
    Function::Function(std::function<double(double)> f, double r1, double r2, Point xy, int count, double xscale,
                       double yscale)
    ...
    Axis::Axis(Orientation d, Point xy, int length, int n, string lab)
        : label(Point{0, 0}, lab),
          line(xy, d == x ? Point{xy.x + length, xy.y} : Point{xy.x, xy.y - length}) {
